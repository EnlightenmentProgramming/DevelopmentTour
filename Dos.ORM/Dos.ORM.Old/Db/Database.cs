...
namespace Dos.ORM
{
    ...
    public sealed class Database : ILogable
    {
        ...
        private DbCommand CreateCommandByCommandType(CommandType commandType, string commandText)
        {
            ...
            string[] connArray = ConnectionString.Split(';');
            if(connArray != null && connArray.Length >0)
            {
                for (int i = 0; i < connArray.Length; i++)
                {
                    if((connArray[i].ToLower()).IndexOf("connect timeout") != -1)
                    {
                        int commandTime;
                        string tempStr = connArray[i].Substring(connArray[i].IndexOf("=")+1);
                        if (int.TryParse(tempStr,out commandTime))
                        {
                            command.CommandTimeout = commandTime;
                            break;
                        }
                    }
                }
