--总增量查询
			...
			select @SubAgent = COUNT(*) from  #AgentTemp where (C_ProA_T>0 or C_ProA>0)
			...
			begin
				select * from #targetTemp where (C_ProA_T>0 or C_ProA>0) -- C_CreTime  between @StartD and @EndD or 
			...
			select @SubAgent = count(*) from #targetTemp where (C_ProA_T > 0 or C_ProA >0)
			...
			select top (@pageSize) * from (
				select * from #targetTemp where (C_ProA_T > 0 or C_ProA >0)
				union all
				select * from #targetTemp1 where (C_CreTime between @StartD and @EndD) or (C_ProA_T > 0 or C_ProA >0)
